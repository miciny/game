...
def switch_and_begin(to_screen):
    ...
    if not start_pos:
        mcy_send_notice(f"第{runtime_setting.current_time}次，第" +
                        str(runtime_setting.current_screen) + "个游戏似乎出错了，没有回到主页！", 2)
        return False
    not_main_check = check_main_true_operation(start_pos)
    if not not_main_check:
        mcy_send_notice(f"第{runtime_setting.current_time}次，第" +
                        str(runtime_setting.current_screen) + "个游戏似乎出错了，没有真的开始！", 2)
        return False
    return True
